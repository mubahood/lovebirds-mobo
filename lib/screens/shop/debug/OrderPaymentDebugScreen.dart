...
class OrderPaymentDebugScreen extends StatefulWidget {
  ...
  @override
  State<OrderPaymentDebugScreen> createState() => _OrderPaymentDebugScreenState();
}
...
class _OrderPaymentDebugScreenState extends State<OrderPaymentDebugScreen> {
  ...
  Future<void> _loadOrders() async {
    try {
      final response = await API.makeApiCall(
        url: API.orders,
        method: 'get',
      );
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        _buildDebugRow('Total Amount:', '\$${order.totalAmount}'),
                        _buildDebugRow('Payment Confirmation:', '${order.paymentConfirmation}'),
                        SizedBox(height: 16),
                        ...
                        Container(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              SizedBox(height: 8),
                              Text('stripePaid != "Yes": ${order.stripePaid != "Yes"}'),
                              Text('stripeUrl.isNotEmpty: ${order.stripeUrl.isNotEmpty}'),
                              Text('stripeUrl.isEmpty: ${order.stripeUrl.isEmpty}'),
                              Text('orderState == 0: ${order.orderState == 0}'),
                              Text('orderState == 1: ${order.orderState == 1}'),
                              SizedBox(height: 8),
                              Text(
                            ...
                            if (order.stripePaid != "Yes" && order.stripeUrl.isNotEmpty)
                              Expanded(
                                child: ElevatedButton.icon(
                                  onPressed: () {
                                    MyWidgets.showToast('Pay Now button clicked');
                                  },
                              ...
                              Expanded(
                                child: ElevatedButton.icon(
                                  onPressed: () {
                                    MyWidgets.showToast('Track Order button clicked');
                                  },
                              ...
                              Expanded(
                                child: ElevatedButton.icon(
                                  onPressed: () {
                                    MyWidgets.showToast('Generate Payment button clicked');
                                  },
                                  icon: Icon(FeatherIcons.creditCard, size: 16),
